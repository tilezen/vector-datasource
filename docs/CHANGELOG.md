...
* **Credits:** [@travisgrigsby](https://github.com/travisgrigsby), [@jeffdefacto](https://github.com/jeffdefacto), [@peitili](https://github.com/peitili), [@iandees](https://github.com/iandees), and [@nvkelso](https://github.com/nvkelso)
  ...
  * **Significant performance improvements** to reduce p99 file sizes globally at all zooms to under 200 kb, with some improvements for low-zoom p50 sizes. This was accomplished with additional geometry simplification, dropping features, dropping properties, and more aggressive merging to multi-lines and multi-polygons, and more sophisticated handling of max zoom and overzoom tiles. Further reductions can be achieved by configuring [Tapalcatl](link_here) to selectively drop layers and name translations (eg to drop `earth` and `transit` layers) depending on your business needs. _Chart shows sizes in bytes (logarithmic scale), based on commonly requested top 100,000 tiles at 512 pixel zoom. NOTE: all other zooms in this document use nominal 256 pixel zooms, offset by 1)._
![tilezen-v1d9-versus-v1d6-size-zooms](https://github.com/tilezen/vector-datasource/raw/master/docs/images/tilezen-v1d9-versus-v1d6-size-zooms.png)
  ...
  * **pois**: Add optional `direction` property (limited to `viewpoint` kinds, and integer indicated degrees from 0-359), thanks [@rwrx](https://github.com/rwrx). (Issue [#598](https://github.com/tilezen/vector-datasource/issues/598) and pull request [#1916](https://github.com/tilezen/vector-datasource/issues/1916))
  ...
  * **places**: Adjust min and max zoom of several country labels, including France and Norway.
  ...
  * **water**: Remove mid-zoom `canal` features when they don't support boats (and indicate if true with new `boat` property), instead show them starting at zoom 11. (Pull request [#2098](https://github.com/tilezen/vector-datasource/pull/2098))
