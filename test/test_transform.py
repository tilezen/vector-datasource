...
class KeepNGriddedTest(unittest.TestCase):
    ...
    def test_points_keep_1_multisort_missing_sort_key(self):
        from tilequeue.process import Context
        import shapely.geometry
        ...
        features = [
            ...
            (test_shape_1, {"foo": "bar", "population": 10000}, "test_shape_1"),
        ]
        feature_layer = dict(
            features=features,
            layer_datum=dict(name='test_layer'),
        ...
        ctx = Context(
            ...
            params=dict(
                ...
                max_items=1,
                grid_size=2,
                sorting_keys=["min_zoom", "population"],
            ),
            resources=None,
            log=None,
        )
        ...
        layer = keep_n_features_gridded(ctx)
        output_features = layer['features']
        self.assertEqual(1, len(output_features),
                         "Should consolidate to a single point in the bucket")
        self.assertEqual("test_shape_2", output_features[0][2],
                         "Should pick the shape with higher population when one sort key is missing")
